...
(async () => {
    ...
    if (!reachable && !force) {
        ...
        skipMsg('two-client coop waitroom shows both peers (signalling blocked)');
        await browser.close();
        server.kill();
    ...
    // Race mode was removed (2026-05-25). The whole "2. → 6." race
    // act-test section that used to live here is gone; coop is now
    // the only run path and is exercised below.

    // ── CO-OP waitroom act test ────────────────────────────────────────
