...
- **Every bugfix and feature ships with a regression test registered in [tests/run.js](tests/run.js).** Prefer a *behavioral* assertion that drives the real code path and would have caught the bug (e.g. orbital-range fires the actual `Tower.update` loop and asserts rockets loose at distance — a pure-math unit test missed the original "never fires distant" bug). `tests/suite-coverage.test.js` fails CI if any `tests/*.test.js` exists but isn't registered, so a test can't silently never-run.
